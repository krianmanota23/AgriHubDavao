...
interface StoreOProfileSettingsProps {
  navigation: any;
  route: any;
}

const StoreO_ProfileSettings: React.FC<StoreOProfileSettingsProps> = ({ navigation, route }) => {
  const [formData, setFormData] = useState<StoreOwnerFormData>({
    firstName: '',
    middleName: '',
    lastName: '',
    userName: '',
  ...
  const handleInputChange = (field: keyof StoreOwnerFormData, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleImageSelection = (option: string) => {
    ...
    if (option === 'remove') {
      ...
      const placeholderImage = 'https://via.placeholder.com/150/2196F3/FFFFFF?text=👤';
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const requiredFields: (keyof StoreOwnerFormData)[] = ['firstName', 'lastName', 'userName', 'storeName', 'email', 'phoneNumber', 'storeLocation'];
          ...
          <View style={styles.content}>
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                onChangeText={(value) => handleInputChange('storeLocation', value)}
                multiline
                numberOfLines={2}
              />
...
export default StoreO_ProfileSettings;
